...
/* // Obsolete this code, we're going to be ~modern~ and use the elevator to get us to undermine.
/obj/effect/step_trigger/teleporter/to_mining/New()
	..()
	teleport_x = src.x
	teleport_y = 2
	teleport_z = Z_LEVEL_UNDERMINE
...
/obj/effect/step_trigger/teleporter/planetary_fall/torris/find_planet()
	planet = planet_torris
...
/obj/effect/step_trigger/lost_in_space/tram
	deathmessage = "You fall off the tram, plummeting through the tree branches before slamming into the ground, the concussive force enough to kill you instantly - if it isn't, the panthers will see to it that you're dead."
...
//Station-unique network cameras
/obj/machinery/camera/network/base
	network = list(NETWORK_BASE)
