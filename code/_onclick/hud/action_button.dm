...
/atom/movable/screen/movable/action_button/proc/save_position()
	...
	LAZYSET(user.client.prefs.action_buttons_screen_locs, "[name]_[id]", position_info)

/atom/movable/screen/movable/action_button/proc/load_position()
	var/mob/user = our_hud.mymob
	if(!user)
		return
	var/position_info = LAZYACCESS(user.client?.prefs?.action_buttons_screen_locs, "[name]_[id]") || SCRN_OBJ_DEFAULT
...
/atom/movable/screen/movable/action_button/proc/dump_save()
	...
	if(!user?.client)
		return
	LAZYREMOVE(user.client.prefs.action_buttons_screen_locs, "[name]_[id]")
