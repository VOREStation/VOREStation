...
/obj/machinery/organ_printer/proc/printing_menu(mob/user)
	...
	visible_message(span_info(span_bold("\The [src]") + " begins churning."))
...
/obj/machinery/organ_printer/proc/can_print(var/choice, var/masscount = 0)
	...
	if(biomass < masscount)
		visible_message(span_info(span_bold("\The [src]") + " displays a warning: 'Not enough biomass. [biomass] stored and [masscount] needed.'"))
		return 0
