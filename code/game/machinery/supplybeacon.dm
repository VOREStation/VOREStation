...
/obj/machinery/power/supply_beacon/process()
	...
	if(!target_drop_time)
		...
		var/drop_y = src.y - 2
		var/drop_z = src.z
		GLOB.command_announcement.Announce("[GLOB.using_map.starsys_name] Rapid Fabrication priority supply request #[rand(1000,9999)]-[rand(100,999)] received. Shipment dispatched via ballistic supply pod for immediate delivery. Have a nice day.", "Thank You For Your Patronage")
		spawn(rand(100, 300))
