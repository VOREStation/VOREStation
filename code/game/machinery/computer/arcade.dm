	...
	if(!blocked && !gameover)
		switch(action)
			if("attack")
				...
				playsound(src, 'sound/arcade/hit.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
				if(turtle > 0)
					turtle--
			...
			if(XENO_CHEM_HEAL)
				...
				playsound(src, 'sound/arcade/heal.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
				turtle++
			...
			if("charge")
				...
				playsound(src, 'sound/arcade/mana.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
				player_mp += chargeamt
				if(turtle > 0)
					turtle--
...
/obj/machinery/computer/arcade/battle/proc/arcade_action(var/mob/user)
	if ((enemy_mp <= 0) || (enemy_hp <= 0))
		if(!gameover)
			...
			playsound(src, 'sound/arcade/win.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		...
		playsound(src, 'sound/arcade/boom.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		player_hp -= boomamt

	else if ((enemy_mp <= 5) && (prob(70)))
		var/stealamt = rand(2,3)
		enemy_action = "[enemy_name] steals [stealamt] of your power!"
		playsound(src, 'sound/arcade/steal.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		player_mp -= stealamt
	...
	if ((player_mp <= 0) || (player_hp <= 0))
		...
		playsound(src, 'sound/arcade/lose.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		if(emagged)
...
/obj/machinery/computer/arcade/orion_trail/attack_hand(mob/living/user)
	...
	if(gameStatus == ORION_STATUS_GAMEOVER)
		playsound(src, 'sound/arcade/ori_fail.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
...
/obj/machinery/computer/arcade/orion_trail/Topic(href, href_list)
	...
	if (href_list["continue"]) //Continue your travels
		...
		if(gameStatus == ORION_STATUS_START)
			playsound(src, 'sound/arcade/ori_begin.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
			newgame(usr)
		...
		if(gameStatus == ORION_STATUS_NORMAL || event == ORION_TRAIL_MUTINY)
			playsound(src, 'sound/arcade/kill_crew.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
	...
	//they also call event() again, to regen the eventdata, which is kind of odd but necessary
		...
		if(gameStatus == ORION_STATUS_MARKET)
			if(!spaceport_raided && food >= 10 && fuel >= 10)
				playsound(src, 'sound/arcade/get_fuel.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		...
		if(gameStatus == ORION_STATUS_MARKET)
			if(!spaceport_raided && settlers.len > 1)
				playsound(src, 'sound/arcade/lose_fuel.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
		...
		if(gameStatus == ORION_STATUS_MARKET)
			if(!spaceport_raided)
				playsound(src, 'sound/arcade/raid.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
...
/obj/machinery/computer/arcade/orion_trail/proc/event()
	...
	switch(event)
		...
		if(ORION_TRAIL_FLUX)
			playsound(src, 'sound/arcade/explo.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
...
/obj/machinery/computer/arcade/orion_trail/proc/win(var/mob/user)
	...
	playsound(src, 'sound/arcade/ori_win.ogg', 50, 1, extrarange = -3, falloff_distance = 0.1, ignore_walls = FALSE)
	if(emagged)
