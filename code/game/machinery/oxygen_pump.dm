...
/obj/machinery/oxygen_pump/attack_hand(mob/user as mob)
	if((stat & MAINT) && tank)
		user.visible_message(span_info(span_bold("\The [user]") + " removes \the [tank] from \the [src]."), span_notice("You remove \the [tank] from \the [src]."))
	...
	if(breather)
		...
		src.visible_message(span_info(span_bold("\The [user]") + " makes \the [contained] rapidly retract back into \the [src]!"))
		if(breather.internals)
			breather.internals.icon_state = "internal0"
		breather = null
...
/obj/machinery/oxygen_pump/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/tank) && (stat & MAINT))
		if(tank)
			...
			W.forceMove(src)
			tank = W
			user.visible_message(span_info(span_bold("\The [user]") + " installs \the [tank] into \the [src]."), span_notice("You install \the [tank] into \the [src]."))
			src.add_fingerprint(user)
