	...
	// OTHER
		...
		for(var/mob/living/silicon/ai/O in living_mob_list)
			if(!O.client)
				continue
			if(U.name == "Unknown")
				to_chat(O, span_info(span_bold("[U]") + " holds \a [itemname] up to one of your cameras ..."))
			else
				to_chat(O, span_info(span_bold("<a href='byond://?src=\ref[O];track2=\ref[O];track=\ref[U];trackname=[U.name]'>[U]</a>") + " holds \a [itemname] up to one of your cameras ..."))
