...
/obj/structure/janitorialcart/proc/equip_janicart_item(mob/user, obj/item/I)
	...
	if(!is_type_in_typecache(I, equippable_item_whitelist))
		user.balloon_alert(user, span_warning("There's no room in [src] for [I]."))
		return FALSE

	if(!user.canUnEquip(I))
		user.balloon_alert(user, span_warning("[I] is stuck to your hand."))
		return FALSE
	...
	if(istype(I, /obj/item/storage/bag/trash))
		...
		if(signs < 4)
			...
			user.balloon_alert(user, span_notice("[src] can't hold any more signs."))
			return FALSE
	else
	...
	update_icon()
	user.balloon_alert(user, span_notice("You put [I] into [src]."))
	return TRUE
		...
		if (mybucket)
			if(I.reagents.total_volume < I.reagents.maximum_volume)
				if(mybucket.reagents.total_volume < 1)
					user.balloon_alert(user, span_notice("[mybucket] is empty!"))
				else
					mybucket.reagents.trans_to_obj(I, 5)	//
					user.balloon_alert(user, span_notice("You wet [I] in [mybucket]."))
					playsound(src, 'sound/effects/slosh.ogg', 25, 1)
			else
				user.balloon_alert(user, span_notice("[I] can't absorb anymore liquid!"))
		else
	...
	switch(action)
		if("bag")
			if(mybag)
				...
				ui.user.balloon_alert(ui.user, span_notice("You take [mybag] from [src]."))
				mybag = null
				nullTguiIcon("mybag")
		...
		if("mop")
			if(mymop)
				...
				ui.user.balloon_alert(ui.user, span_notice("You take [mymop] from [src]."))
				mymop = null
				nullTguiIcon("mymop")
			else if(is_type_in_typecache(I, equippable_item_whitelist))
				equip_janicart_item(ui.user, I)
		if("spray")
			if(myspray)
				...
				ui.user.balloon_alert(ui.user, span_notice("You take [myspray] from [src]."))
				myspray = null
		...
		if("replacer")
			if(myreplacer)
				...
				ui.user.balloon_alert(ui.user, span_notice("You take [myreplacer] from [src]."))
				myreplacer = null
		...
		if("sign")
			if(istype(I, /obj/item/clothing/suit/caution) && signs < 4)
				...
				if(sign)
					...
					ui.user.balloon_alert(ui.user, span_notice("You take \a [sign] from [src]."))
					signs--
					if(!signs)
						nullTguiIcon("signs")
			else
				ui.user.balloon_alert(ui.user, span_notice("[src] doesn't have any signs left."))
		if("bucket")
			if(mybucket)
				mybucket.forceMove(get_turf(ui.user))
				ui.user.balloon_alert(ui.user, span_notice("You unmount [mybucket] from [src]."))
				mybucket = null
