...
/obj/structure/decor/vv_edit_var(var_name, var_value)
	if(var_name == "state")
 ...
 */
// sheetizer from /tg/
/obj/structure/decor/stamper
	icon_state = "stamper"

/obj/structure/decor/stamper/change_state(state)
	. = ..()
	switch(state)
		if("off")
			cut_overlays()
...
/obj/structure/decor/stamper/starts_on
	icon_state = "stamper_on"

/obj/structure/decor/stamper/starts_on/Initialize()
	add_overlay("stamper_proc")
	add_overlay("stamper_but")
...
/obj/structure/decor/nt_pod/Initialize(mapload)
...
/obj/structure/decor/nt_pod/Entered(atom/movable/mover)
	abduct(mover)

/obj/structure/decor/nt_pod/proc/abduct(var/atom/movable/AM)
	// Save old settings
...
/obj/structure/decor/nt_pod/proc/unduct(var/atom/movable/AM)
	// Geddout
	vis_contents -= AM
	if(ismob(AM))
		var/mob/M = AM
...
/obj/structure/decor/nt_pod/MouseDrop_T(var/atom/movable/AM, var/mob/user)
	if(contents.len)
		return
	if(!ismovable(AM))
		return
...
/obj/structure/decor/nt_pod/change_state(state)
	. = ..()
	switch(state)
		if("open")
