...
/obj/structure/grille/bullet_act(var/obj/item/projectile_new/Proj)
	if(!Proj)	return
	...
	switch(Proj.shot_data.damage_type)
		if(BRUTE)
			...
			if(Proj.original == src || prob(20))
				Proj.shot_data.damage *= between(0, Proj.shot_data.damage/60, 0.5)
				if(prob(max((damage-10)/25, 0))*100)
					passthrough = TRUE
			else
				Proj.shot_data.damage *= between(0, Proj.shot_data.damage/60, 1)
				passthrough = TRUE
		if(BURN)
			//beams and other projectiles are either blocked completely by grilles or stop half the damage.
			if(!(Proj.original == src || prob(20)))
				Proj.shot_data.damage *= 0.5
				passthrough = TRUE

	if(passthrough)
		. = PROJECTILE_CONTINUE
		damage = between(0, (damage - Proj.shot_data.damage)*(Proj.shot_data.damage_type == BRUTE? 0.4 : 1), 10) //if the bullet passes through then the grille avoids most of the damage
