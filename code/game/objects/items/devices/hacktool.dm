...
/obj/item/device/multitool/hacktool/proc/attempt_hack(var/mob/user, var/atom/target)
	...
	if(!is_type_in_list(target, supported_types))
		to_chat(user, "[bicon(src)] <span class='warning'>Unable to hack this target, invalid target type.</span>")
		return 0

	var/obj/machinery/door/airlock/D = target
	if(D.security_level > max_level)
		to_chat(user, "[bicon(src)] <span class='warning'>Target's electronic security is too complex.</span>")
		return 0
