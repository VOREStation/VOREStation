...
/mob/proc/changeling_generic_armor(var/armor_type, var/helmet_type, var/boot_type, var/chem_cost)
	...
	if(M.inventory.get_item_in_slot(slot_head_str) || wear_suit) //Make sure our slots aren't full
...
/mob/proc/changeling_generic_equip_all_slots(var/list/stuff_to_equip, var/cost)
	...
	if(M.mind.changeling.armor_deployed)
		if(stuff_to_equip["head"])
			if(istype(M.inventory.get_item_in_slot(slot_head_str), stuff_to_equip["head"]))
				qdel(M.inventory.get_item_in_slot(slot_head_str))
				success = 1
		...
		if(!M.inventory.get_item_in_slot(slot_head_str) && t)
			var/I = new t
