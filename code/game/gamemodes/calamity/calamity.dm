...
/datum/game_mode/calamity/create_antagonists()
	...
	shuffle(SSantags.antag_datums) // This is probably the only instance in the game where the order will be important.
	var/i = 1
	var/grab_antags = round(num_players()/ANTAG_TYPE_RATIO)+1
	for(var/antag_id in SSantags.antag_datums)
		if(i > grab_antags)
			break
		antag_tags |= antag_id
		i++
