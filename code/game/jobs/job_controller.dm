...
/datum/controller/occupations/proc/AssignRole(var/mob/new_player/player, var/rank, var/latejoin = 0)
	...
	if(player && player.mind && rank)
		...
		if(!job)
			return 0
		if((job.minimum_character_age || job.min_age_by_species) && (player.read_preference(/datum/preference/numeric/age) < job.get_min_age(player.client.prefs.species, player.client.prefs.organ_data["brain"])))
			return 0
...
/datum/controller/occupations/proc/FindOccupationCandidates(datum/job/job, level, flag)
	...
	for(var/mob/new_player/player in unassigned)
		...
		if(!job.player_old_enough(player.client))
			...
			continue
		if(job.minimum_character_age && (player.read_preference(/datum/preference/numeric/age) < job.get_min_age(player.client.prefs.species, player.client.prefs.organ_data["brain"])))
	...
	for(var/level = 1 to 3)
		for(var/command_position in SSjob.get_job_titles_in_department(DEPARTMENT_COMMAND))
			...
			for(var/mob/V in candidates)
				...
				var/age = V.read_preference(/datum/preference/numeric/age)
