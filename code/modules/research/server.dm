...
/obj/machinery/computer/rdservercontrol/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		...
		if("reset_tech")
			...
			if(!istype(target))
				return FALSE
			var/choice = alert(usr, "Technology Data Rest", "Are you sure you want to reset this technology to its default data? Data lost cannot be recovered.", "Continue", "Cancel")
			if(choice == "Continue")
		...
		if("reset_design")
			...
			if(!istype(target))
				return FALSE
			var/choice = alert(usr, "Design Data Deletion", "Are you sure you want to delete this design? If you still have the prerequisites for the design, it'll reset to its base reliability. Data lost cannot be recovered.", "Continue", "Cancel")
			if(choice == "Continue")
