	...
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("change_id")
				...
				if(attempt_code == access_code)
					eftpos_name = sanitize(tgui_input_text(usr, "Enter a new terminal ID for this device", "Enter new EFTPOS ID",max_length=MAX_NAME_LEN), MAX_NAME_LEN, encode = FALSE) + " EFTPOS scanner"
					print_reference()
				else
					to_chat(usr, "[icon2html(src, usr.client)]" + span_warning("Incorrect code entered."))
			if("link_account")
			...
			if("trans_purpose")
				var/choice = sanitize(tgui_input_text(usr, "Enter reason for EFTPOS transaction", "Transaction purpose"), encode = FALSE)
				if(choice)
