...
/datum/event/lore_news/announce()
	...
	var/list/tajara = list("Meralar", "Mesomori", "Arathiir")
	...
	"All SolCom citizens beware, the scourge know as the Vox have increased the frequency of their attacks on isolated and poorly defended SolCom territories in [pick(bowl)]. Citizens have been advised to travel in groups and remain aware of their surroundings at all times.",
	"The Unathi have recently moved a large amount of material into Abel's Rest, under the premise of building additional power plants. According to our sources, the material is in fact being channeled into domestic terrorist groups on both sides, in an attempt to incite a SolCom response. More news as the story develops.",
	...
	"Nova Sixty, Mercurial web personality, claims that the disappearence of the SCG-R Song Shi was a plot by \"extradimensional rockmen\" who intend to sow mistrust between the Association and SolCom. Quote Sixty, \"The Far Kingdoms' attempt to hide the TRUTH only proves that they and their rockman allies are NOT to be trusted.\"",
