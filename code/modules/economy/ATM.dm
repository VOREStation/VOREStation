...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/weapon/card))
		if(emagged > 0)
			...
			to_chat(user, span_red("[bicon(src)] CARD READER ERROR. This system has been compromised!"))
			return
		...
		switch(href_list["choice"])
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num)
							if(number_incorrect_tries > max_pin_attempts)
								...
								to_chat(usr, span_red("[bicon(src)] Incorrect pin/account combination entered, [max_pin_attempts - number_incorrect_tries] attempts remaining."))
								previous_account_number = tried_account_num
								playsound(src, 'sound/machines/buzz-sigh.ogg', 50, 1)
						else
							to_chat(usr, span_red("[bicon(src)] incorrect pin/account combination entered."))
							number_incorrect_tries = 0
					else
						...
						to_chat(usr, span_blue("[bicon(src)] Access granted. Welcome user '[authenticated_account.owner_name].'"))
			...
			if("insert_card")
				if(!held_card)
					...
					if(emagged > 0)
						to_chat(usr, span_red("[bicon(src)] The ATM card reader rejected your ID because this machine has been sabotaged!"))
					else
