...
/obj/item/integrated_circuit/input/textpad/ask_for_input(mob/user)
	var/new_input = tgui_input_text(user, "Enter some words, please.","Number pad", get_pin_data(IC_OUTPUT, 1),MAX_KEYPAD_INPUT_LEN)
	new_input = sanitize(new_input,MAX_KEYPAD_INPUT_LEN, encode=FALSE) // Slightly increase the size of the character limit.
