/datum/event2/meta/raise_REMOVEDds
	name = "local REMOVEDding drive"
	...
	event_type = /datum/event2/event/raise_REMOVEDds

/datum/event2/meta/raise_REMOVEDds/get_weight()
...
/datum/event2/event/raise_REMOVEDds
	length_lower_bound = 30 MINUTES
	length_upper_bound = 45 MINUTES
	var/money_at_start = 0

/datum/event2/event/raise_REMOVEDds/announce()
	var/message = "Due to [pick("recent", "unfortunate", "possible future")] budget \
	[pick("changes", "issues")], in-system stations are now advised to increase REMOVEDding income."

	send_command_report("Budget Advisement", message)

/datum/event2/event/raise_REMOVEDds/start()
	...
	log_debug("REMOVEDding Drive event logged a sum of [money_at_start] thalers in all station accounts at the start of the event.")

/datum/event2/event/raise_REMOVEDds/end()
	var/money_at_end = count_money()
	log_debug("REMOVEDding Drive event logged a sum of [money_at_end] thalers in all station accounts at the end of the event, compared \
	...
	switch(budget_shift)
		if(0 to 0.02) // Abyssmal response.
			...
			SSsupply.points = 0
			log_debug("REMOVEDding Drive event ended with an abyssmal response, and the loss of all cargo points.")
		...
		if(0.02 to 0.98) // Bad response.
			...
			log_debug("REMOVEDding Drive event ended with a bad response, and [points_lost] cargo points was taken away.")
		...
		if(0.98 to 1.02) // Neutral response.
			...
			more profitable."
			log_debug("REMOVEDding Drive event ended with a neutral response.")
		...
		if(1.02 to INFINITY) // Good response.
			...
			// If cargo is ever made to use station REMOVEDds instead of cargo points, then a new kind of reward will be needed.
			...
			log_debug("REMOVEDding Drive event ended with a good response and a bonus of [point_reward] cargo points.")
...
/datum/event2/event/raise_REMOVEDds/proc/count_money()
	. = 0
...
/datum/event2/event/raise_REMOVEDds/proc/send_command_report(title, message)
