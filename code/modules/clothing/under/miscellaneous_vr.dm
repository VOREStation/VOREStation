...
/obj/item/clothing/gloves/bluespace/deluxe/proc/bluespace_size(user) //Taken from HYPER suit
	if(!ishuman(user))
		return
	...
	var/new_size = tgui_input_number(user, "Put the desired size you wish to be while wearing the bracelet ([RESIZE_SMALL*100]-[RESIZE_BIG*100]%).", "Set Size", RESIZE_NORMAL*100, RESIZE_BIG*100, RESIZE_SMALL*100)
	if(!new_size)
		return
