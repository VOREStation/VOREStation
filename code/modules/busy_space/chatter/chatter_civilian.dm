...
/datum/atc_chatter/pathwarning/squak()
	...
	switch(phase)
		if(1)
			...
			atc.msg("[combined_first_name], [callname]. Your [pick("ship","vessel","starship")] is approaching [navhazard], observe minimum safe distance and adjust your heading appropriately.")
			next()
		if(2)
			var/confirm = pick("Understood","Roger that","Affirmative","Our bad","Thanks for the heads up")
			atc.msg("[confirm] [callname], adjusting course.","[comm_first_name]")
			next()
		else
			atc.msg("Your compliance is appreciated, [combined_first_name].")
			qdel(src)
...
/datum/atc_chatter/civvieleaks/squak()
	...
	switch(phase)
		if(1)
			...
			switch(commleak)
				if("thatsmywife")
					atc.msg("-so then I says to him, |that's no [pick("space carp","space shark","vox","garbage scow","freight liner","cargo hauler","superlifter")], that's my +wife!+| And he-","[comm_first_name]")
					next()
				if("missingkit")
					atc.msg("-did you get the kit from down on deck [rand(1,4)]? I need th-","[comm_first_name]")
					next()
				if("pipeleaks")
					atc.msg("I swear if these pipes keep leaking I'm going to-","[comm_first_name]")
					next()
				if("weirdsmell")
					atc.msg("-and where the hell is that smell coming fr-","[comm_first_name]")
					next()
				if("weirdsmell2")
					atc.msg("-hat in the [pick("three","five","seven","nine")] hells did you |eat| [pick("ensign","crewman")]? This compartment reeks of-","[comm_first_name]")
					next()
				if("scug")
					atc.msg("-and if that weird cat of yours keeps crawling into the pipes we-","[comm_first_name]")
					next()
				if("teppi")
					atc.msg("-at are we supposed to do with this damn cow?","[comm_first_name]")
					next(1,PROC_REF(teppi_next)) // Someone had to be a snowflake
		if(2)
			atc.msg("[combined_first_name], your internal comms are leaking[pick("."," again.",", again.",". |Again|.")]")
			next()
		else
			atc.msg("Sorry Control, won't happen again.","[comm_first_name]")
			qdel(src)

/datum/atc_chatter/civvieleaks/proc/teppi_next()
	atc.msg("I don't think it's a cow, sir, it looks more like a-","[comm_first_name]")
	next()

/datum/atc_chatter/slogan/squak()
	switch(phase)
		if(1)
			atc.msg("The following is a sponsored message from [name].","Facility PA")
			next()
		else
			atc.msg("[slogan]","Facility PA")
			qdel(src)
...
/datum/atc_chatter/misc/squak()
	...
	switch(phase)
		if(1)
			atc.msg("[callname], [combined_first_name] on [mission] [pick(mission_noun)] to [destname], requesting [request].","[comm_first_name]")
			next()
		if(2)
			atc.msg("[combined_first_name], [callname], [response].")
			next()
		else
			atc.msg("[callname], [yes ? "thank you" : "understood"], out.","[comm_first_name]")
			qdel(src)
