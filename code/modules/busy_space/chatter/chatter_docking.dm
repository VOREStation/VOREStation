...
/datum/atc_chatter/report_to_dock/squak()
	...
	switch(phase)
		if(1)
			GLOB.atc.msg("This is [using_map.dock_name] Tower. Would a free [situation_type] team please report to [landing_zone] immediately. This is not a drill.")
			next()
		else
			GLOB.atc.msg("Repeat, any free [situation_type] team, report to [landing_zone] immediately. This is +not+ a drill.")
			finish()
...
/datum/atc_chatter/dockingrequestgeneric/squak()
	...
	switch(request_type)
		if("generic")
			switch(phase)
				if(1)
					GLOB.atc.msg("[callname], [combined_first_name], [pick("stopping by","passing through")] on our way to [destname], requesting permission to [landing_short].","[comm_first_name]")
					next()
				if(2)
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
					next()
				else
					GLOB.atc.msg("[appreciation], [callname]. [dockingplan]","[comm_first_name]")
					finish()
		...
		if("delayed")
			switch(phase)
				...
				if(2)
					...
					GLOB.atc.msg("[combined_first_name], [callname]. Request denied, [reason] and resubmit your request.")
					next()
				if(3)
					GLOB.atc.msg("Understood, [callname].","[comm_first_name]")
					next(60)
				if(4)
					GLOB.atc.msg("[callname], [combined_first_name], resubmitting [landing_move].","[comm_first_name]")
					next()
				else
					GLOB.atc.msg("[combined_first_name], [callname]. Everything appears to be in order now, permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
					finish()
		...
		if("supply")
			switch(phase)
				if(1)
					...
					GLOB.atc.msg("[callname], [combined_first_name]. We're [preintensifier][intensifier] low on [low_thing]. Requesting permission to [landing_short] for resupply.","[comm_first_name]")
					next()
				else
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
					finish()
		...
		if("repair")
			switch(phase)
				if(1)
					...
					appreciation = pick("Much appreciated","Many thanks","Understood","You're a lifesaver","We owe you one","I owe you one","Perfect, thank you")
					GLOB.atc.msg("[callname], [combined_first_name]. [damagestate]. Requesting permission to [landing_short] for repairs and maintenance.","[comm_first_name]")
					next()
				else
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Repair crews are standing by, contact them on channel [GLOB.atc.engchannel].")
					finish()
		...
		if("medical")
			switch(phase)
				if(1)
					...
					appreciation = pick("Much appreciated","Many thanks","Understood","You're a lifesaver","We owe you one","I owe you one","Perfect, thank you")
					GLOB.atc.msg("[callname], [combined_first_name]. We have [medicalstate] on board. Requesting permission to [landing_short] for medical assistance.","[comm_first_name]")
					next()
				else
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Medtechs are standing by, contact them on channel [GLOB.atc.medchannel].")
					finish()
		...
		if("security")
			switch(phase)
				if(1)
					...
					GLOB.atc.msg("[callname], [combined_first_name]. We have [securitystate] on board and require security assistance. Requesting permission to [landing_short].","[comm_first_name]")
					next()
				else
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Security teams are standing by, contact them on channel [GLOB.atc.secchannel].")
					finish()
...
/datum/atc_chatter/undockingrequest/squak()
	...
	switch(phase)
		if(1)
			...
			GLOB.atc.msg("[callname], [combined_first_name], requesting permission to [takeoff] from [landing_zone].","[comm_first_name]")
			next()
		if(2)
			var/request_type = pick(150;"generic",50;"delayed")
			switch(request_type)
				if("generic")
					GLOB.atc.msg("[combined_first_name], [callname]. Permission granted. Docking clamps released. [safetravels].")
					next()
				if("delayed")
					...
					GLOB.atc.msg("Negative [combined_first_name], request denied. [denialreason]. Try again in a few minutes.")
			...
			GLOB.atc.msg("[thanks], [callname]. This is [combined_first_name] setting course for [destname], out.","[comm_first_name]")
			finish()

/datum/atc_chatter/undockingrequest/proc/delay_1()
	GLOB.atc.msg("[callname], [combined_first_name], re-requesting permission to depart from [landing_zone].","[comm_first_name]")
	next(1,PROC_REF(delay_2))

/datum/atc_chatter/undockingrequest/proc/delay_2()
	GLOB.atc.msg("[combined_first_name], [callname]. Everything appears to be in order now, permission granted. Docking clamps released. [safetravels].")
	next()
...
/datum/atc_chatter/undockingdenied/squak()
	...
	switch(phase)
		if(1)
			...
			GLOB.atc.msg("Negative [combined_first_name], request denied. [denialreason].")
			finish()
