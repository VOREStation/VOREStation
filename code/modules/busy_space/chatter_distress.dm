...
/datum/atc_chatter/distress/squak()
	...
	switch(state)
		if("calm")
			switch(phase)
				if(1)
					SSatc.msg("[GLOB.using_map.starsys_name] Defense Control, [combined_first_name].","[comm_first_name]")
					next()
				if(2)
					SSatc.msg("[combined_first_name], [GLOB.using_map.starsys_name] Defense Control.","[GLOB.using_map.starsys_name] Defense Control")
					next()
				if(3)
					...
					next()
				if(4)
					SSatc.msg("[combined_first_name], [GLOB.using_map.starsys_name] Defense Control copies. Sensor data matches logged profile for [secondprefix] |[secondshipname]|. SDF units are en route to your location.","[GLOB.using_map.starsys_name] Defense Control")
					next()
				else
		...
		if("panic")
			switch(phase)
				if(1)
					...
					next()
				if(2)
					SSatc.msg("[combined_first_name], [GLOB.using_map.starsys_name] Defense Control. SDF is en route, contact on [SSatc.sdfchannel].")
					next()
				else
					SSatc.msg("[pick("Copy that","Understood")] [GLOB.using_map.starsys_name] Defense Control, switching now!","[comm_first_name]")
					finish()
