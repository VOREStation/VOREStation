...
/datum/reagent/toxin/phoron/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	if(alien == IS_VOX)
		M.adjustOxyLoss(-75 * removed) //Half as efficent as dex+ would be.
		return //You're wasting plasma (a semi-limited chemical) to save someone, so it might as well be strong.
	..()
