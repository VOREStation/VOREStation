...
/datum/event/radiation_storm/tick()
	if(activeFor == enterBelt)
		GLOB.command_announcement.Announce("The [GLOB.using_map.facility_type] has entered the radiation belt. Please remain in a sheltered area until we have passed the radiation belt.", "Anomaly Alert")
		radiate()
	...
	if(postStartTicks == radIntervall)
		...
		GLOB.command_announcement.Announce("The [GLOB.using_map.facility_type] has passed the radiation belt. Please allow for up to one minute while radiation levels dissipate, and report to medbay if you experience any unusual symptoms. Maintenance will lose all access again shortly.", "Anomaly Alert")
/datum/event/radiation_storm/proc/radiate()
	...
	for(var/mob/living/carbon/C in GLOB.living_mob_list)
		if(!(C.z in GLOB.using_map.station_levels) || C.isSynthetic() || isbelly(C.loc))
			continue
