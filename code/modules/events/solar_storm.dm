...
/datum/event/solar_storm/announce()
	GLOB.command_announcement.Announce("A solar storm has been detected approaching \the [station_name()]. Please halt all EVA activites immediately and return to the interior of the [GLOB.using_map.facility_type].", "Anomaly Alert", new_sound = 'sound/AI/radiation.ogg')
...
/datum/event/solar_storm/start()
	GLOB.command_announcement.Announce("The solar storm has reached the [GLOB.using_map.facility_type]. Please refrain from EVA and remain inside the station until it has passed.", "Anomaly Alert")
...
/datum/event/solar_storm/end()
	GLOB.command_announcement.Announce("The solar storm has passed the [GLOB.using_map.facility_type]. It is now safe to resume EVA activities. Please report to medbay if you experience any unusual symptoms. ", "Anomaly Alert")
	adjust_solar_output()
