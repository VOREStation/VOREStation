...
/datum/controller/subsystem/events/Topic(href, href_list)
	...
	if(href_list["toggle_report"])
		...
		if(alert(usr, "Stopping an event may have unintended side-effects. Continue?","Stopping Event!","Yes","No") != "Yes")
			return
		...
		if(alert(usr, "This will remove the event from rotation. Continue?","Removing Event!","Yes","No") != "Yes")
			return
		...
		if(!new_event.name || !new_event.event_type)
			return
		if(alert(usr, "This will add a new event to the rotation. Continue?","Add Event!","Yes","No") != "Yes")
			return
