...
/datum/computer_file/program/wordprocessor/tgui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("PRG_newfile")
			...
			var/newname = sanitize(tgui_input_text(ui.user, "Enter file name:", "New File"), encode = FALSE)
			if(!newname)
				return TRUE
		...
		if("PRG_saveasfile")
			var/newname = sanitize(tgui_input_text(ui.user, "Enter file name:", "Save As"), encode = FALSE)
			if(!newname)
				return TRUE
		...
		if("PRG_savefile")
			if(!open_file)
				open_file = sanitize(tgui_input_text(ui.user, "Enter file name:", "Save As"), encode = FALSE)
				if(!open_file)
					return 0
