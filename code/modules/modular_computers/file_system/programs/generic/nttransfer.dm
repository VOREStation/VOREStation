	...
	switch(action)
		if("PRG_downloadfile")
			...
			if(remote.server_password)
				var/pass = sanitize(tgui_input_text(ui.user, "Code 401 Unauthorized. Please enter password:", "Password required"), encode = FALSE)
		...
		if("PRG_reset")
			...
			return TRUE
		if("PRG_setpassword")
			var/pass = sanitize(tgui_input_text(ui.user, "Enter new server password. Leave blank to cancel, input 'none' to disable password.", "Server security", "none"), encode = FALSE)
			if(!pass)
				return
			if(pass == "none")
				server_password = ""
