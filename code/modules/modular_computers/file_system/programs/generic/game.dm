...
/datum/computer_file/program/game/proc/game_check(mob/user)
	...
	if(boss_hp <= 0)
		...
		playsound(computer.loc, 'sound/arcade/win.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
		game_active = FALSE
		...
		playsound(computer.loc, 'sound/arcade/lose.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
		game_active = FALSE
		program_icon_state = "arcade_off"
		if(istype(computer))
			computer.update_icon()
...
/datum/computer_file/program/game/proc/enemy_check(mob/user)
	...
	if(boss_mp <= 5)
		...
		playsound(computer.loc, 'sound/arcade/steal.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
		player_mp -= boss_mpamt
		boss_mp += boss_mpamt
	else if(boss_mp > 5 && boss_hp <12)
		heads_up = "[boss_name] heals for [bossheal] health!"
		playsound(computer.loc, 'sound/arcade/heal.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
		boss_hp += bossheal
		boss_mp -= boss_mpamt
	else
		heads_up = "[boss_name] attacks you for [boss_attackamt] damage!"
		playsound(computer.loc, 'sound/arcade/hit.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
...
/datum/computer_file/program/game/tgui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("Recharge_Power")
			...
			playsound(computer.loc, 'sound/arcade/mana.ogg', 50, TRUE, extrarange = -3, falloff_distance = 0.1)
