...
/datum/computer_file/program/records/tgui_act(action, list/params, datum/tgui/ui)
	if(..())
		return TRUE
...
/datum/computer_file/program/records/tgui_data(mob/user)
	...
	if(active_record)
		...
		data["creation"] = TRUE
		data["dnasearch"] = TRUE
		data["fingersearch"] = TRUE

	return data

/datum/computer_file/program/records/proc/get_record_access(var/mob/user)
	//var/list/user_access = using_access || user.GetAccess()
	var/list/user_access = user.GetAccess()

	var/obj/PC = tgui_host()
...
/datum/computer_file/program/records/proc/edit_field(var/mob/user, var/field_ID)
	var/datum/computer_file/report/crew_record/R = active_record
	if(!R)
		return
	...
	if(!F.verify_access_edit(get_record_access(user)))
		to_chat(user, "<span class='notice'>\The [tgui_host()] flashes an \"Access Denied\" warning.</span>")
		return
	F.ask_value(user)

/datum/computer_file/program/records/proc/get_photo(var/mob/user)
