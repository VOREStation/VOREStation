	...
	switch(action)
		...
		if("appearance")
			...
			if(choice && tgui_status(ui.user, state) == STATUS_INTERACTIVE)
				...
				. = TRUE
		if("assignment")
			var/new_job = sanitize(tgui_input_text(ui.user,"What assignment would you like to put on this card?\nChanging assignment will not grant or remove any access levels.","Agent Card Assignment", S.assignment), encode = FALSE)
		...
		if("bloodtype")
			...
			var/new_blood_type = sanitize(tgui_input_text(ui.user,"What blood type would you like to be written on this card?","Agent Card Blood Type",default), encode = FALSE)
		...
		if("dnahash")
			...
			var/new_dna_hash = sanitize(tgui_input_text(ui.user,"What DNA hash would you like to be written on this card?","Agent Card DNA Hash",default), encode = FALSE)
		...
		if("fingerprinthash")
			...
			var/new_fingerprint_hash = sanitize(tgui_input_text(ui.user,"What fingerprint hash would you like to be written on this card?","Agent Card Fingerprint Hash",default), encode = FALSE)
		...
		if("photo")
			...
			. = TRUE
		if("sex")
			var/new_sex = sanitize(tgui_input_text(ui.user,"What sex would you like to put on this card?","Agent Card Sex", S.sex), encode = FALSE)
			if(!isnull(new_sex) && tgui_status(ui.user, state) == STATUS_INTERACTIVE)
				S.sex = new_sex
				to_chat(ui.user, span_notice("Sex changed to '[new_sex]'."))
				. = TRUE
		if("species")
			var/new_species = sanitize(tgui_input_text(ui.user,"What species would you like to put on this card?","Agent Card Species", S.species), encode = FALSE)
