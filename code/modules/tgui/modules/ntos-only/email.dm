...
/datum/tgui_module/email_client/tgui_act(action, params, datum/tgui/ui)
	...
	switch(action)
		...
		if("changepassword")
			var/oldpassword = sanitize(tgui_input_text(ui.user,"Please enter your old password:", "Password Change", null, 100), 100, encode = FALSE)
			if(!oldpassword)
				return 1
			var/newpassword1 = sanitize(tgui_input_text(ui.user,"Please enter your new password:", "Password Change", null, 100), 100, encode = FALSE)
			if(!newpassword1)
				return 1
			var/newpassword2 = sanitize(tgui_input_text(ui.user,"Please re-enter your new password:", "Password Change", null, 100), 100, encode = FALSE)
			if(!newpassword2)
				return 1
		...
		if("save")
			...
			var/filename = sanitize(tgui_input_text(ui.user,"Please specify file name:", "Message export", null, 100), 100, encode = FALSE)
			if(!filename)
				return 1
