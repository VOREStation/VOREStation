...
/datum/mentor_help/New(msg, client/C)
	...
	to_chat(C, "<span class='notice'>PM to-<b>Mentors</b>: [name]</span>")
...
/client/verb/mentorhelp(msg as text)
	...
	spawn(1200)
		src.verbs += /client/verb/mentorhelp	// 2 minute cool-down for adminhelps
	...
	if(current_mentorhelp)
		if(tgui_alert(usr, "You already have a ticket open. Is this for the same issue?","Duplicate?",list("Yes","No")) != "No")
			if(current_mentorhelp)
				...
				to_chat(usr, "<span class='adminnotice'>PM to-<b>Mentors</b>: [msg]</span>")
				return
			else
