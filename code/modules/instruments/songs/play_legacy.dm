	...
	for(var/mob/M as anything in hearing_mobs)
		...
		*/
		if(!M)
			hearing_mobs -= M
			continue
		M.playsound_local(source, null, volume * using_instrument.volume_multiplier, sound_to_use = music_played, preference = /datum/preference/toggle/instrument_toggle, volume_channel = VOLUME_CHANNEL_INSTRUMENTS)
