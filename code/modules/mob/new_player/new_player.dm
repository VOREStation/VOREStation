...
/mob/new_player/proc/LateChoices()
	...
	for(var/datum/job/job in job_master.occupations)
		if(job && IsJobAvailable(job.title))
			...
			if((job.minimum_character_age || job.min_age_by_species) && (client.prefs.read_preference(/datum/preference/numeric/age) < job.get_min_age(client.prefs.species, client.prefs.organ_data["brain"])))
				continue
