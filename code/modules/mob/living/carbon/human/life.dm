	...
	if(chem_effects[CE_ALLERGEN])
		...
		if(species.allergen_reaction & AG_GIBBING)
			if(prob(disable_severity / 8))
				...
				emote(pick(list("whimper","belch","belch","belch","choke","shiver")))
				Weaken(disable_severity / 3)
		if(species.allergen_reaction & AG_SNEEZE)
			if(prob(disable_severity/3))
				if(prob(20))
					to_chat(src, span_warning("You go to sneeze, but it gets caught in your sinuses!"))
				else if(prob(80))
					if(prob(30))
						to_chat(src, span_warning("You feel like you are about to sneeze!"))
					addtimer(CALLBACK(src, PROC_REF(allergy_sneeze)), rand(0.75,2) SECOND, TIMER_DELETE_ME)
...
/mob/living/carbon/human/proc/allergy_gib(var/remaining)
	SHOULD_NOT_OVERRIDE(TRUE)
	PRIVATE_PROC(TRUE)
	if(remaining > 0)
		emote(pick(list("whimper","belch","shiver")))
		remaining--
		addtimer(CALLBACK(src, PROC_REF(allergy_gib), remaining), rand(1,1.2) SECOND, TIMER_DELETE_ME)
		return
	emote("belch")
	gib()
