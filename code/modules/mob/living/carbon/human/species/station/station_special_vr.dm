...
/datum/species/chimera
	...
	icobase = 'icons/mob/human_races/r_sergal.dmi' //Sergal basis, as it has working skin color. Replace with human when sprites are edited & available.
	...
	flags = NO_SCAN //Dying as a chimera is, quite literally, a death sentence.
...
/datum/species/chimera/handle_environment_special(var/mob/living/carbon/human/H) //Actually handles hunger.
	...
	if(H.nutrition > 100 && H.species.has_fine_manipulation == 0) //If they went feral then ate something.
		...
		H << "<span class='primary'>You feel as if you're no longer feral, the feeling of hunger having now passed.</span>"
		return
