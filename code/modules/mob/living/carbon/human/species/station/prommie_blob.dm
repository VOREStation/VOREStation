...
/mob/living/simple_mob/slime/promethean/proc/toggle_expand()
	...
	if(is_wide)
		is_wide = FALSE
		src.visible_message(span_info(span_bold("[src.name]") + " pulls together, compacting themselves into a small ball!"))
		update_icon()
	else
		is_wide = TRUE
		src.visible_message(span_info(span_bold("[src.name]") + " flows outwards, their goop expanding!"))
		update_icon()
...
/mob/living/simple_mob/slime/promethean/proc/toggle_shine()
	...
	if(shiny)
		shiny = FALSE
		src.visible_message(span_info(span_bold("[src.name]") + " dulls their shine, becoming more translucent."))
		update_icon()
	else
		shiny = TRUE
		src.visible_message(span_info(span_bold("[src.name]") + " glistens and sparkles, shining brilliantly."))
		update_icon()
...
/mob/living/carbon/human/proc/prommie_intoblob(force)
	...
	//Message
	blob.visible_message(span_info(span_bold("[src.name]") + " squishes into their true form!"))
...
/mob/living/carbon/human/proc/prommie_outofblob(var/mob/living/simple_mob/slime/promethean/blob, force)
	...
	//Message
	blob.visible_message(span_info(span_bold("[src.name]") + " pulls together, forming a humanoid shape!"))
