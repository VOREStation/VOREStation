/datum/trait/aquatic
	...
	custom_only = FALSE
	excludes = list(/datum/trait/good_swimmer, /datum/trait/bad_swimmer, /datum/trait/aquatic/plus)
...
/datum/trait/aquatic/plus
	...
	allowed_species = list(SPECIES_HANNER, SPECIES_CUSTOM, SPECIES_REPLICANT_CREW) //So it only shows up for custom species, hanner, and Gamma replicants
	custom_only = FALSE
	excludes = list(/datum/trait/good_swimmer, /datum/trait/bad_swimmer, /datum/trait/aquatic)
	category = TRAIT_TYPE_POSITIVE
...
/datum/trait/good_swimmer
	...
	excludes = list(/datum/trait/bad_swimmer)
