...
/datum/unarmed_attack/bite/sharp/numbing/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	var/datum/gender/T = gender_datums[target.get_visible_gender()]

	attack_damage = CLAMP(attack_damage, 1, 5)
	if(target == user)
		user.visible_message("<span class='danger'>[user] [pick(attack_verb)] [U.himself] in the [affecting.name]!</span>")
	...
	switch(zone)
		if(BP_HEAD, O_MOUTH, O_EYES)
			...
			switch(attack_damage)
				...
				if(5)
					user.visible_message("<span class='danger'>[user] sinks [U.his] [pick(attack_noun)] <b><i>deep</i></b> into [target]'s neck, causing the vein to bulge outwards at some type of chemical is pumped into it!</span>")
			...
			switch(attack_damage)
				...
				if(5)
					user.visible_message("<span class='danger'>[user]'s fangs sink deep into [target]'s [affecting.name], one of [T.his] veins bulging outwards from the sudden fluid pumped into it!</span>")
