	...
	if(attack_damage >= 5 && armour < 2 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if(BP_L_ARM, BP_L_HAND)
				if (target.get_left_hand())
					// Disarm left hand
					//Urist McAssistant dropped the macguffin with a scream just sounds odd.
					target.visible_message(span_danger("\The [target.get_left_hand()] was knocked right out of [target]'s grasp!"))
					target.drop_l_hand()
			if(BP_R_ARM, BP_R_HAND)
				if (target.get_right_hand())
					// Disarm right hand
					target.visible_message(span_danger("\The [target.get_right_hand()] was knocked right out of [target]'s grasp!"))
					target.drop_r_hand()
			if(BP_TORSO)
				if(!target.lying)
