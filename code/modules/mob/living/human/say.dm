<<<<<<< HEAD:code/modules/mob/living/carbon/human/say.dm
...
/mob/living/human/say_understands(var/mob/other, var/datum/language/speaking = null)
	...
	//These only pertain to common. Languages are handled by mob/say_understands()
	if(!speaking)
		if(istype(other, /mob/living/simple_mob/diona_nymph))
			if(other.languages.len >= 2) //They've sucked down some blood and can speak common now.
				return TRUE
		if(issilicon(other))
			return TRUE
...
/mob/living/human/binarycheck()
	...
	if(istype(r_ear, /obj/item/radio/headset))
		...
		if(R.translate_binary)
			. = TRUE
>>>>>>> 666428014d2... Merge pull request #8546 from Atermonera/surgery_refactor:code/modules/mob/living/human/say.dm
