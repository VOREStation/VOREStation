...
*/
	...
	if(air_master.current_cycle%2==1 && length(touchable_items))
		...
		for(var/mob/living/carbon/human/T in (touchable_items))
			if((T.status_flags & GODMODE)) //|| !T.digestable) //Remove the comment and the second ) next to the godmode. Need the digestable varaible added in, first.
		...
		//Handle the target being anything but a /mob/living/carbon/human
			...
			//if //Change to "else" when items_presevered is added in, and reenable the lines above.
