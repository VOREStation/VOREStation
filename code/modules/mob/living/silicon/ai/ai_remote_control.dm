	...
	for(var/mob/living/silicon/robot/R as anything in GLOB.available_ai_shells)
		if(R.shell && !R.deployed && (R.stat != DEAD) && (!R.connected_ai || (R.connected_ai == src) ) )	//VOREStation Edit: shell restrictions
			if(istype(R.loc, /obj/machinery/recharge_station))	//Check Rechargers
				...
				if(!(GLOB.using_map.ai_shell_restricted && !(RS.z in GLOB.using_map.ai_shell_allowed_levels)))	//Allow station borgs to be redeployed from Chargers.
					possible += R

			if(isbelly(R.loc))	//check belly space
				var/obj/belly/B = R.loc
				if(!(GLOB.using_map.ai_shell_restricted && !(B.owner.z in GLOB.using_map.ai_shell_allowed_levels)))	//No smuggling in borgs
					possible += R

			if(!(GLOB.using_map.ai_shell_restricted && !(R.z in GLOB.using_map.ai_shell_allowed_levels)))
				possible += R
