...
/mob/living/bot/secbot/UnarmedAttack(var/mob/M, var/proximity)
	...
	if(ishuman(M))
		...
		if(!cuff)
			...
			if(do_mob(src, H, 60))
				if(!H.handcuffed)
					if(istype(H.inventory.get_item_in_slot(slot_back_str), /obj/item/rig) && istype(H.inventory.get_item_in_slot(slot_gloves_str),/obj/item/clothing/gloves/gauntlets/rig))
