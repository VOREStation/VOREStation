...
/mob/living/voice/proc/transfer_identity(mob/speaker)
	if(ismob(speaker))
		...
		if(speaker.client && speaker.client.prefs)
			...
			name = p.read_preference(/datum/preference/text/human/real_name)
			real_name = name
...
/mob/living/voice/verb/change_name()
	...
	var/new_name = sanitizeSafe(tgui_input_text(src, "Who would you like to be now?", "Communicator", src.client.prefs.read_preference(/datum/preference/text/human/real_name), MAX_NAME_LEN, encode = FALSE), MAX_NAME_LEN)
	if(new_name)
		if(comm)
