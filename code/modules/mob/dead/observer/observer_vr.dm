/mob/observer/dead/verb/nifjoin()
	...
	var/picked = tgui_input_list(usr, "Pick a friend with NIF and Soulcatcher to join into. Harrass strangers, get banned. Not everyone has a NIF w/ Soulcatcher.","Select a player", GLOB.player_list)
...
/mob/observer/dead/verb/findghostpod() //Moves the ghost instead of just changing the ghosts's eye -Nodrak
	...
	var/input = tgui_input_list(usr, "Select a ghost pod:", "Ghost Jump", observe_list_format(GLOB.active_ghost_pods))
	if(!input)
