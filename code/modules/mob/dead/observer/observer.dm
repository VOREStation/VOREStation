...
/mob/observer/dead/check_holy(var/turf/T)
	if(check_rights(R_ADMIN|R_REMOVED|R_EVENT, 0, src))
		return 0
...
/mob/observer/dead/verb/bloody_doodle()
	...
	if(!ghosts_can_write && !check_rights(R_ADMIN|R_EVENT|R_REMOVED, 0)) //Let's allow for admins to write in blood for events and the such.
