...
/mob/observer/dead/proc/checkStatic()
	return !(check_rights_for(src, R_ADMIN|R_FUN|R_EVENT|R_SERVER) || (client && client.buildmode) || isbelly(loc))
...
/mob/living/verb/ghost()
	...
	if(stat == DEAD && !forbid_seeing_deadchat)
		...
		var/response
		if(check_rights_for(src, R_ADMIN|R_SERVER|R_MOD)) //No need to sanity check for client and holder here as that is part of check_rights
...
/mob/observer/dead/check_holy(var/turf/T)
	if(check_rights_for(src, R_ADMIN|R_FUN|R_EVENT))
		return 0
...
/mob/observer/dead/proc/can_admin_interact()
	return check_rights_for(src, R_ADMIN|R_EVENT)
