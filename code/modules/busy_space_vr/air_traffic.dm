...
/datum/lore/atc_controller
	var/delay_min = 1 MINUTE //45 MINUTES			//How long between ATC traffic, minimum
	var/delay_max = 2 MINUTES //90 MINUTES			//Ditto, maximum
	...
	var/initial_delay = 2 MINUTES //15 MINUTES			//How long to wait before sending the first message of the shift.
	var/next_message = 1 MINUTE //45 MINUTES			//When the next message should happen in world.time - Making it default to min value
...
/datum/lore/atc_controller/proc/random_convo(var/force_chatter_type)
	...
	var/firstid = "[rand(1,9)][rand(0,9)][rand(0,9)][rand(0,9)]"
	...
	//var/secondowner = secondary.short_name	//ditto
	var/secondprefix = pick(secondary.ship_prefixes)	//Pick a random prefix
	var/secondid = "[rand(1,9)][rand(0,9)][rand(0,9)][rand(0,9)]"
	...
	switch(chatter_type)
		...
		if("emerg")
			...
			msg("+Mayday, mayday, mayday!+ [combined_first_name], declaring an emergency! [problem]!","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[combined_first_name], [callname]. Switch to emergency responder channel [ertchannel].")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[callname], [combined_first_name] switching now.","[prefix] [shipname]")
		...
		if("policescan")
			...
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[confirm] [callname], holding position.","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("policeshipscan")
			...
			var/completed = pick("You're free to proceed.","Everything looks fine, carry on.","You're clear. Move along.","Apologies for the delay, you're clear.","Switch to channel [sdfchannel] and await further instruction.")
			msg("[combined_second_name], [combined_first_name], your [pick("ship","vessel","starship")] has been flagged for routine inspection. Hold position and prepare to be scanned.","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			...
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY)*3 SECONDS)
			msg("[combined_second_name], [combined_first_name]. Scan complete. [completed]","[prefix] [shipname]")
		...
		if("policeshipflee")
			...
			msg("Unknown [pick("ship","vessel","starship")], [combined_second_name], identify yourself and submit to a full inspection. Flying without an active transponder is a violation of interstellar shipping regulations.","[secondprefix] [secondshipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[uhoh]","Unknown Vessel")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[using_map.starsys_name] Defense Control,  [combined_second_name]. We have a [prefix] here, please advise.","[secondprefix] [secondshipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("policeshipcombat")
			...
			msg("[using_map.starsys_name] Defense Control,  [combined_second_name], engaging [combined_first_name] [pick("near route","in sector")] [rand(1,100)], [battlestatus]","[secondprefix] [secondshipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("sdfpatrolupdate")
			...
			msg("[using_map.starsys_name] Defense Control,  [combined_first_name] reporting in, [statusupdate], over.","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("sdfendingpatrol")
			...
			msg("[callname], [combined_first_name], returning from our system patrol route, requesting permission to [landing_short].","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[appreciation], [callname]. [dockingplan]","[prefix] [shipname]")
		...
		if("sdfchatter")
			...
			switch(chain)
				if("codecheck")
					...
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
					msg("[callname], confirm auth-code... [rand(1,9)][rand(1,9)][rand(1,9)]-[pick("Alpha","Beta","Gamma","Delta","Epsilon","Zeta","Eta","Theta","Iota","Kappa","Lambda","Mu","Nu","Xi","Omicron","Pi","Rho","Sigma","Tau","Upsilon","Phi","Chi","Psi","Omega")]?","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
					msg("One moment...")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY)*2 SECONDS)
		...
		if("distress")
			...
			switch(state)
				if("calm")
					msg("[using_map.starsys_name] Defense Control, [combined_first_name].","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
				...
				if("panic")
					msg("+Mayday, mayday, mayday!+ This is [combined_first_name] declaring an emergency! We are under attack! Requesting immediate assistance!","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("pathwarning")
			...
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[confirm] [callname], adjusting course.","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("dockingrequestgeneric")
			...
			switch(request_type)
				if("generic")
					msg("[callname], [combined_first_name], [pick("stopping by","passing through")] on our way to [destname], requesting permission to [landing_short].","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
					msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				if("delayed")
					var/reason = pick(
					...
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
					msg("Understood, [callname].","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY)*60 SECONDS)
					msg("[callname], [combined_first_name], resubmitting [landing_move].","[prefix] [shipname]")
					sleep (5 SECONDS)
				...
				if("supply")
					...
					msg("[callname], [combined_first_name]. We're [preintensifier][intensifier] low on [low_thing]. Requesting permission to [landing_short] for resupply.","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
					msg("[combined_first_name], [callname]. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				if("repair")
					...
					appreciation = pick("Much appreciated","Many thanks","Understood","You're a lifesaver","We owe you one","I owe you one","Perfect, thank you")
					msg("[callname], [combined_first_name]. [damagestate]. Requesting permission to [landing_short] for repairs and maintenance.","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
				...
				if("medical")
					...
					appreciation = pick("Much appreciated","Many thanks","Understood","You're a lifesaver","We owe you one","I owe you one","Perfect, thank you")
					msg("[callname], [combined_first_name]. We have [medicalstate] on board. Requesting permission to [landing_short] for medical assistance.","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
				...
				if("security")
					...
					msg("[callname], [combined_first_name]. We have [securitystate] on board and require security assistance. Requesting permission to [landing_short].","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
		...
		if("undockingrequest")
			...
			msg("[callname], [combined_first_name], requesting permission to [takeoff] from [landing_zone].","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			switch(request_type)
				if("generic")
				...
				if("delayed")
					...
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY)*60 SECONDS)
					msg("[callname], [combined_first_name], re-requesting permission to depart from [landing_zone].","[prefix] [shipname]")
					sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			...
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[thanks], [callname]. This is [combined_first_name] setting course for [destname], out.","[prefix] [shipname]")
		...
		if("sdfbeginpatrol")
			...
			msg("[callname], [combined_first_name], requesting permission to [takeoff] from [landing_zone] to begin system patrol.","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[combined_first_name], [callname]. Permission granted. Docking clamps released. [safetravels].")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[thanks], [callname]. This is [combined_first_name] beginning system patrol, out.","[prefix] [shipname]")
		...
		if("slogan")
			...
			msg("[callname], [combined_first_name] on [mission] [pick(mission_noun)] to [destname], requesting [request].","[prefix] [shipname]")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[combined_first_name], [callname], [response].")
			sleep(rand(MIN_MSG_DELAY,MAX_MSG_DELAY) SECONDS)
			msg("[callname], [yes ? "thank you" : "understood"], out.","[prefix] [shipname]")
