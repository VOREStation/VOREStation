	...
	switch(action)
		...
		if("startup")
			if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0))
				activated = TRUE
				visible_message(span_blue("[bicon(src)] [src] whirrs."), span_blue("[bicon(src)]You hear something whirr."))
	...
	if(activated)
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			...
			if(inserted_battery.stored_charge <= 0)
				src.loc.visible_message(span_blue("[bicon(src)] [src] buzzes."), span_blue("[bicon(src)] You hear something buzz."))
				shutdown_emission()
			else if(world.time > time_end)
				src.loc.visible_message(span_blue("[bicon(src)] [src] chimes."), span_blue("[bicon(src)] You hear something chime."))
				shutdown_emission()
		else
			src.visible_message(span_blue("[bicon(src)] [src] buzzes."), span_blue("[bicon(src)] You hear something buzz."))
			shutdown_emission()
		last_process = world.time
