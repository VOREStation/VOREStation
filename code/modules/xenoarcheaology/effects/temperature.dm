...
/datum/artifact_effect/temperature/DoEffectTouch(var/mob/user) //This still exists in case you VV it.
	...
	if(holder)
		...
		if(env)
			...
			if(temp_change == HOT && env.get_temp() < target_temp)
				temp_coef = ((target_temp)/(env.get_temp()+1)) //TT = 300. ET = 250. TC = ~1 || TT = 3000. ET = 300 TT = 10. We multiply by 10 below to speed it up.
				env.set_temp(max(env.get_temp() + temp_coef*10, TCMB))
			else if(temp_change == COLD && env.get_temp() > target_temp)
				temp_coef = (env.get_temp()/(target_temp+1)) //ET = 300, TT = 25. TC = 12. Next: ET=288 TT = 25, TC = 11.52. ETC.
				env.set_temp(max(env.get_temp() - temp_coef, TCMB))
