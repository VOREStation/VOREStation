	...
	if(client)
		switch(emote_sound_mode)
			if(EMOTE_SOUND_NO_FREQ)
				playsound(T, SFX_EMOTE, 75, TRUE, falloff_distance = 1, frequency = 0, ignore_walls = TRUE, preference = /datum/preference/toggle/emote_sounds)
			if(EMOTE_SOUND_VOICE_FREQ)
				playsound(T, SFX_EMOTE, 75, TRUE, falloff_distance = 1, frequency = voice_freq, ignore_walls = TRUE, preference = /datum/preference/toggle/emote_sounds)
			if(EMOTE_SOUND_VOICE_LIST)
				playsound(T, pick(voice_sounds_list), 75, TRUE, falloff_distance = 1, frequency = voice_freq, ignore_walls = TRUE, preference = /datum/preference/toggle/emote_sounds)
