/obj/item/weapon/research_sample
	...
	desc = "A curious sample of unknown material. Perhaps a scientist could tell you more about it? <span class='warning'>It looks dangerous to handle without heavy gloves or other protective equipment.</span>"
	...
	var/fixed_tech = null	//do we have a predetermined tech-group, per request? if so, overrides randomization
	...
	persist_storable = FALSE //don't shove hazardous shinies into the item bank!! also their properties are randomized on creation, so saving them is pointless-- you won't get out what you put in
	...
	var/handle_danger	= 80	//20% chance to hurty if you handle it wrong; lower is riskier
...
/obj/item/weapon/research_sample/New()
	...
	else	//if we're not a preset, randomize the name and associated tech, to make sure samples aren't predictable/metagamable
		name = "[pick("strange","anomalous","exotic","atypical","unusual","incongruous","weird","aberrant","eccentric")] [pick("object","sample","thing","fragment","specimen","element","alloy","chunk","remnant","scrap","sliver")]"
		rand_tech = pick(valid_techs)
		origin_tech.Add(list("[rand_tech]" = tech_value))
		var/sample_icon = rand(1,10)
		icon_state = "generic_sample[sample_icon]"
		/*
		//per-state effects, like glows/light emission (if desired)
		switch(sample_icon)
			if(1)
				..
			if(2)
...
/obj/item/weapon/research_sample/attack_hand(mob/user)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(istype(G) && (G.permeability_coefficient < 0.25) || prob(handle_danger) || istype(G, /obj/item/clothing/gloves/gauntlets))
			burn_user = FALSE
