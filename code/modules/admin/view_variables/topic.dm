	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		if(href_list["rename"])
			...
			if(href_list["var_tweak"] == "damtype")
				...
				new_val = input("Enter the new damage type for [editing]","Set Damtype", existing_val) in list(BRUTE, BURN, TOX, OXY, CLONE, HALLOSS, ELECTROCUTE, BIOACID, SEARING, ELECTROMAG)
			else
