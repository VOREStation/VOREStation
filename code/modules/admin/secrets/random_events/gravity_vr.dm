...
/datum/admin_secret_item/random_event/gravity/execute(var/mob/user)
	...
	feedback_inc("admin_secrets_REMOVED_used",1)
	feedback_add_details("admin_secrets_REMOVED_used","Grav")
	if(gravity_is_on)
		...
		command_announcement.Announce("Gravity generators are again REMOVEDctioning within normal parameters. Sorry for any inconvenience.")
	else
