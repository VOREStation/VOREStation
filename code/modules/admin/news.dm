...
/proc/paper_markup2html(var/text)
	...
	text = replacetext(text, "\[logo\]", "<img src = [SSassets.transport.get_asset_url("ntlogo.png")]>") // Not sure if these would get used but why not
	text = replacetext(text, "\[sglogo\]", "<img src = [SSassets.transport.get_asset_url("sglogo.png")]>")
	return text
...
/proc/html2paper_markup(var/text)
	...
	text = replacetext(text, "<img src = [SSassets.transport.get_asset_url("ntlogo.png")]>", "\[logo\]") // Not sure if these would get used but why not
	text = replacetext(text, "<img src = [SSassets.transport.get_asset_url("sglogo.png")]>", "\[sglogo\]")
	return text
