...
/datum/lua_editor/tgui_static_data(mob/user)
	...
	data["auxtools_enabled"] = TRUE
	data["ss_lua_init"] = SSlua.subsystem_initialized
	return data

/datum/lua_editor/tgui_data(mob/user)
	var/list/data = list()
	//if(!CONFIG_GET(flag/auxtools_enabled) || !SSlua.subsystem_initialized)
	if(!SSlua.subsystem_initialized)
		return data
