...
/obj/item/gun/magnetic/attackby(var/obj/item/thing, var/mob/user)
	...
	if(removable_components)
		if(istype(thing, /obj/item/cell))
			...
			user.visible_message(span_info(span_bold("\The [user]") + " slots \the [cell] into \the [src]."))
			update_icon()
			return
		...
		if(thing.has_tool_quality(TOOL_SCREWDRIVER))
			...
			user.visible_message(span_info(span_bold("\The [user]") + " unscrews \the [capacitor] from \the [src]."))
		...
		if(istype(thing, /obj/item/stock_parts/capacitor))
			...
			playsound(src, 'sound/machines/click.ogg', 10, 1)
			power_per_tick = (power_cost*0.15) * capacitor.rating
			user.visible_message(span_info(span_bold("\The [user]") + " slots \the [capacitor] into \the [src]."))
			update_icon()
			return
	...
	if(istype(thing, load_type))
		...
		user.visible_message(span_info(span_bold("\The [user]") + " loads \the [src] with \the [loaded]."))
...
/obj/item/gun/magnetic/attack_hand(var/mob/user)
	if(user.get_inactive_hand() == src)
		...
		if(removing)
			...
			user.visible_message(span_info(span_bold("\The [user]") + " removes \the [removing] from \the [src]."))
			playsound(src, 'sound/machines/click.ogg', 10, 1)
			update_icon()
			return
	. = ..()
