...
/obj/item/gun_new/dropped(var/mob/living/user)
	if(istype(user))
		user.stop_aiming(src)
	return ..()

/obj/item/gun_new/equipped(var/mob/living/user, var/slot)
...
/obj/item/gun_new/proc/PreFire(var/atom/A, var/mob/living/user, var/params)
	if(!user.aiming)
