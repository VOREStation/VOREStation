...
/obj/structure/table/attackby(obj/item/W, mob/user)
	...
	if(carpeted && W.has_tool_quality(TOOL_CROWBAR))
		user.visible_message(span_info(span_bold("\The [user]") + " removes the carpet from \the [src]."),
	...
	if(!carpeted && material && istype(W, /obj/item/stack/tile/carpet))
		...
		if(C.use(1))
			user.visible_message(span_info(span_bold("\The [user]") + " adds \the [C] to \the [src]."),
	...
	if(health < maxhealth && W.has_tool_quality(TOOL_WELDER))
		...
		if(F.welding)
			...
			if(!do_after(user, 20 * F.toolspeed) || !F.remove_fuel(1, user))
				return
			user.visible_message(span_info(span_bold("\The [user]") + " repairs some damage to \the [src]."),
...
/obj/structure/table/attack_generic(mob/user as mob, var/damage)
	if(damage >= 10)
		if(reinforced && prob(70))
			...
			user.do_attack_animation(src)
			return 1
	visible_message(span_info(span_bold("\The [user]") + " scratches at \the [src]!"))
	return ..()
...
/obj/structure/table/proc/common_material_remove(mob/user, datum/material/M, delay, what, type_holding, sound)
	...
	if(manipulating) return M
	manipulating = 1
	user.visible_message(span_info(span_bold("\The [user]") + " begins removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place."),
	...
	if(!do_after(user, delay))
		manipulating = 0
		return M
	user.visible_message(span_info(span_bold("\The [user]") + " removes the [M.display_name] [what] from \the [src]."),
...
/obj/structure/table/proc/dismantle(obj/item/W, mob/user)
	...
	manipulating = 1
	user.visible_message(span_info(span_bold("\The [user]") + " begins dismantling \the [src]."),
	                              span_notice("You begin dismantling \the [src]."))
	playsound(src, W.usesound, 50, 1)
	if(!do_after(user, 20 * W.toolspeed))
		manipulating = 0
		return
	user.visible_message(span_info(span_bold("\The [user]") + " dismantles \the [src]."),
