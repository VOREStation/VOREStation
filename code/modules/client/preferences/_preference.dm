...
/// Please use SScharacter_setup.queue_preferences_save(prefs) when you edit multiple at once and set direct_write to FALSE
/mob/proc/write_preference_directly(preference_type, preference_value, direct_write = TRUE)
	var/success = client?.prefs?.write_preference_by_type(preference_type, preference_value, direct_write)
	if(success)
...
/datum/preferences/proc/write_preference_by_type(preference_type, preference_value, direct_write = FALSE)
	var/datum/preference/preference_entry = GLOB.preference_entries[preference_type]
	if(isnull(preference_entry))
	...
	if(preference_entry.savefile_identifier == PREFERENCE_CHARACTER && !direct_write)
