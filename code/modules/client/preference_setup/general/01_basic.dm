...
/datum/category_item/player_setup_item/general/basic/tgui_data(mob/user)
	...
	var/list/species_stats = list(
		...
		"cold_level_1" = FLOOR(max(0,species.cold_level_1)-T0C,1),
		"heat_level_1" = FLOOR(max(0,species.heat_level_1)-T0C,1),
		...
		"body_temperature" = FLOOR(max(0,species.body_temperature)-T0C,1),
		"hazard_low_pressure" = FLOOR(max(0,species.hazard_low_pressure),1),
		"breath_type" = GLOB.gas_data.name[species.breath_type] ? GLOB.gas_data.name[species.breath_type] : "NA",
		"hazard_high_pressure" = species.hazard_high_pressure == INFINITY ? "∞" : FLOOR(max(0,species.hazard_high_pressure),1),
