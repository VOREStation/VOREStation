...
/datum/fish_trait/proc/apply_to_mob(mob/living/mob)
	SHOULD_CALL_PARENT(TRUE)
...
/datum/fish_trait/shiny_lover/difficulty_mod(obj/item/fishing_rod/rod, mob/fisherman)
	...
	if(HAS_TRAIT(rod, TRAIT_ROD_ATTRACT_SHINY_LOVERS) || (rod.bait?.get_item_credit_value() >= 50))
		.[ADDITIVE_FISHING_MOD] -= FISH_TRAIT_MINOR_DIFFICULTY_BOOST
...
/datum/fish_trait/nocturnal/apply_to_mob(mob/living/mob)
	. = ..()
...
/datum/fish_trait/heavy/catch_weight_mod(obj/item/fishing_rod/rod, mob/fisherman, atom/location, obj/item/fish/fish_type)
	. = ..()
	var/datum/material/material = rod.get_primary_material()
	if(!material)
		return
...
/datum/fish_trait/heavy/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/carnivore/catch_weight_mod(obj/item/fishing_rod/rod, mob/fisherman, atom/location, obj/item/fish/fish_type)
	. = ..()
	if(istype(rod.get_primary_material(), /datum/material/flesh)) //who cares about the bait, that fishing rod is yummy!
		return
	if(!rod.bait)
		.[MULTIPLICATIVE_FISHING_MOD] = 0
		return
...
/datum/fish_trait/vegan/catch_weight_mod(obj/item/fishing_rod/rod, mob/fisherman, atom/location, obj/item/fish/fish_type)
	. = ..()
	if(istype(rod.get_primary_material(), /datum/material/fibers)) //bamboo is technically grass.
		return
	if(!rod.bait)
		.[MULTIPLICATIVE_FISHING_MOD] = 0
		return
	if(HAS_TRAIT(rod.bait, TRAIT_OMNI_BAIT))
		return
	if(istype(rod.bait, /obj/item/reagent_containers/food/snacks/grown))
		return
...
/datum/fish_trait/emulsijack/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
	RegisterSignal(mob, COMSIG_LIVING_HANDLE_BREATHING, PROC_REF(on_non_stasis_life))
...
/datum/fish_trait/emulsijack/psychic/on_non_stasis_life(mob/living/simple_mob/mob, seconds_per_tick)
	return
...
/datum/fish_trait/revival/proc/revive(datum/weakref/fish_ref)
	...
	var/message = span_green("[source] twitches. It's alive!")
	if(source.loc && HAS_TRAIT(source.loc, TRAIT_IS_AQUARIUM))
		source.loc.visible_message(message)
	else
		source.visible_message(message)

/datum/fish_trait/revival/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/toxic/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/lubed/catch_weight_mod(obj/item/fishing_rod/rod, mob/fisherman, atom/location, obj/item/fish/fish_type)
	. = ..()
	if(istype(rod.get_primary_material(), /datum/material/bananium)) //x5 chance of catching lubefish & co with a bananium rod.
...
/datum/fish_trait/lubed/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/amphibious/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/antigrav/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/electrogenesis/proc/on_force_updated(obj/item/fish/fish, weight_rank, bonus_or_malus)
	...
	if(fish.status == FISH_ALIVE)
		...
		fish.damtype = BURN
		fish.attack_verb = list("shocks", "zaps")
//		fish.attack_verb_simple = list("shock", "zap")
		fish.hitsound = 'sound/effects/sparks4.ogg'

/datum/fish_trait/electrogenesis/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/stunted/apply_to_mob(mob/living/simple_mob/mob)
	. = ..()
...
/datum/fish_trait/ink/remove_from_fish(obj/item/fish/fish)
	. = ..()
	UnregisterSignal(fish, list(COMSIG_ITEM_ATTACK_ZONE)) //COMSIG_ATOM_PROCESSED
...
/datum/fish_trait/ink/proc/attacked_someone(obj/item/fish/source, mob/living/target, mob/living/user, zone)
	...
	if(HAS_TRAIT(source, TRAIT_FISH_INK_ON_COOLDOWN) || source.status == FISH_DEAD)
		return
	if(!iscarbon(target) || target.get_organ(BP_HEAD))
		target.AdjustBlinded(rand(2,4))
		target.AdjustConfused(rand(1,2))
		/*
	...
	playsound(target, pick(SFX_DESECRATION), 50, TRUE)
