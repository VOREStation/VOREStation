		...
		if(prob(chance))
			for(var/mob/living/L as anything in buckled_mobs)
				if(!(user in buckled_mobs))
					L.visible_message(\
					span_info(span_bold("\The [user]") + " frees \the [L] from \the [src]."),\
					span_info(span_bold("\The [user]") + " frees you from \the [src]."),\
					span_warning("You hear shredding and ripping."))
				else
					L.visible_message(\
					span_info(span_bold("\The [L]") + " struggles free of \the [src]."),\
