...
/obj/item/hand/attackby(obj/O as obj, mob/user as mob)
	if(cards.len == 1 && istype(O, /obj/item/pen))
		...
		if(P.name != "Blank Card")
			...
			return
		var/cardtext = sanitize(tgui_input_text(user, "What do you wish to write on the card?", "Card Editing", null, MAX_PAPER_MESSAGE_LEN), MAX_PAPER_MESSAGE_LEN, encode = FALSE)
		if(!cardtext)
			return
		P.name = cardtext
