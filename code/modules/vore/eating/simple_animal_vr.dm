...
/mob/living/simple_mob/proc/toggle_digestion()
	...
	set desc = "Enables digestion on this mob for three minutes."
	set category = "OOC"
	set src in oview(1)
	...
	if(vore_selected.digest_mode == DM_HOLD)
		var/confirm = tgui_alert(user, "Enabling digestion on [name] will cause it to digest all stomach contents. Using this to break OOC prefs is against the rules. Digestion will reset after three minutes.", "Enabling [name]'s Digestion", "Enable", "Cancel")
		if(confirm == "Enable")
			vore_selected.digest_mode = DM_DIGEST
			addtimer(VARSET_CALLBACK(vore_selected, digest_mode, vore_default_mode), 3 MINUTES)
	else
		var/confirm = tgui_alert(user, "This mob is currently set to process all stomach contents. Do you want to disable this?", "Disabling [name]'s Digestion", "Disable", "Cancel")
		if(confirm == "Disable")
