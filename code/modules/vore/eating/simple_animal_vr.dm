...
/mob/living/simple_mob/proc/toggle_digestion()
	...
	if(vore_selected.digest_mode == DM_HOLD)
		var/confirm = alert(user, "Enabling digestion on [name] will cause it to digest all stomach contents. Using this to break OOC prefs is against the rules. Digestion will reset after three minutes.", "Enabling [name]'s Digestion", "Enable", "Cancel")
		if(confirm == "Enable")
