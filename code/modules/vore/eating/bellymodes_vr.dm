...
///////////////////// Early Non-Mode Handling /////////////////////
			...
			if(ishuman(L))
				...
				if(mode_flags & DM_FLAG_STRIPPING)
					for(var/slot in slots)
						...
						if(I)
							H.unEquip(I, force = FALSE)// We always contaminate IDs.
							if(contaminates || istype(I, /obj/item/weapon/card/id))
								I.gurgle_contaminate(H, contamination_flavor, contamination_color)
							...
							switch(item_digest_mode)
								...
								if(IM_DIGEST_FOOD)
									if(istype(I,/obj/item/weapon/reagent_containers/food) || istype(I,/obj/item/organ))
										...
										items_preserved |= I
									if(prob(25)) //Less often than with normal digestion
										if(H && H.client && H.is_preference_enabled(/datum/client_preference/digestion_noises))
											SEND_SOUND(H, prey_digest)
										play_sound = pick(pred_digest)
								if(IM_DIGEST)
									if(!H && (I.digest_stage && I.digest_stage > 0))
										digest_item(I)
									else
										digest_item(I)
										did_an_item = TRUE
	...
	var/digestion_noise_chance = FALSE
	var/steal_nutrition = FALSE
	...
	switch(digest_mode)
		...
		if(DM_ABSORB)
			for(var/target in touchable_mobs)
				var/mob/living/M = target
				if(!M.absorbable || M.absorbed)
					continue
				digestion_noise_chance = 10
				steal_nutrition = TRUE
				if(M.nutrition < 100)
					absorb_living(M)
					to_update = TRUE
		if(DM_UNABSORB)
		...
		if(DM_SHRINK)
			...
			for(var/target in touchable_mobs)
				var/mob/living/M = target
				if(M.size_multiplier > shrink_grow_size)
