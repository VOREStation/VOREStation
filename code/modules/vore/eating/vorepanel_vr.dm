...
/datum/vore_look/proc/gen_ui(var/mob/living/user)
	...
	if(!selected)
		...
		dat += "<br><a href='byond://?src=\ref[src];b_nutritionpercent=\ref[selected]'>Nutritional Gain:</a>"
	...
	switch(user.absorbable)
		if(TRUE)
			dat += "<a style='background:#173d15;' href='byond://?src=\ref[src];toggleabsorbable=1'>Toggle Absorbtion Permission (Currently: ON)</a>"
		if(FALSE)
			dat += "<a style='background:#990000;' href='byond://?src=\ref[src];toggleabsorbable=1'>Toggle Absorbtion Permission (Currently: OFF)</a>"
	...
	switch(user.allowmobvore)
		if(TRUE)
			dat += "<br><a style='background:#173d15;' href='byond://?src=\ref[src];togglemv=1'>Toggle Mob Vore (Currently: ON)</a>"
		if(FALSE)
			dat += "<br><a style='background:#990000;' href='byond://?src=\ref[src];togglemv=1'>Toggle Mob Vore (Currently: OFF)</a>"
	...
	dat += "<a href='byond://?src=\ref[src];togglenoisy=1'>Toggle Hunger Noises</a>"

	dat += "<HR>"

	//Under the last HR, save and stuff.
	dat += "<a href='byond://?src=\ref[src];saveprefs=1'>Save Prefs</a>"
	dat += "<a href='byond://?src=\ref[src];refresh=1'>Refresh</a>"
	dat += "<a href='byond://?src=\ref[src];applyprefs=1'>Reload Slot Prefs</a>"
...
/datum/vore_look/proc/vp_interact(href, href_list)
	...
	if(href_list["toggledg"])
		...
		message_admins("[key_name(user)] toggled their digestability to [user.digestable] ([user ? "<a href='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[user.loc.];Y=[user.loc.y];Z=[user.loc.z]'>JMP</a>" : "null"])")
	...
	if(href_list["togglemv"])
		...
		message_admins("[key_name(user)] toggled their mob vore preference to [user.allowmobvore] ([user ? "<a href='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[user.loc.];Y=[user.loc.y];Z=[user.loc.z]'>JMP</a>" : "null"])")
