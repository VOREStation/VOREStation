...
/datum/controller/subsystem/cameras/Initialize()
	update_offsets(world.maxz) // SSmapping.max_plane_offset
...
/datum/controller/subsystem/cameras/proc/update_offsets(new_offset)
	for(var/i in length(obscured_images) to new_offset)
		...
		obscured.plane = PLANE_FULLSCREEN
...
/datum/controller/subsystem/cameras/proc/update_eye_chunk(mob/observer/eye/eye)
...
ADMIN_VERB(pause_camera_updates, R_ADMIN, "Toggle Camera Updates", "Stop security cameras from updating, meaning what they see now is what they will see forever.", ADMIN_CATEGORY_DEBUG)
	...
	log_and_message_admins("[SScameras.disable_camera_updates ? "disabled" : "enabled"] camera updates.", user)
	feedback_add_details("admin_verb","TCameraU") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
