...
/datum/controller/subsystem/mobs/stat_entry(msg)
	msg = "P: [GLOB.mob_list.len] | S: [slept_mobs]"
	return ..()
...
/datum/controller/subsystem/mobs/proc/log_recent()
	...
	if(!currentrun || !GLOB.mob_list)
		...
		log_world(msg)
		return
	msg += "Lists: currentrun: [currentrun.len], GLOB.mob_list: [GLOB.mob_list.len]\n"

	if(!currentrun.len)
		msg += "!!The subsystem just finished the GLOB.mob_list list, and currentrun is empty (or has never run).\n"
		msg += "!!The info below is the tail of GLOB.mob_list instead of currentrun.\n"
	...
	if(!position)
		msg += "Unable to find context of tail entry in GLOB.mob_list list.\n"
	else
		if(position != GLOB.mob_list.len)
			var/additional = GLOB.mob_list.Find(D, position+1)
			if(additional)
				msg += "WARNING: Tail entry found more than once in GLOB.mob_list list! Context is for the first found.\n"
		var/start = clamp(position-2,1,GLOB.mob_list.len)
		var/end = clamp(position+2,1,GLOB.mob_list.len)
		msg += "2 previous elements, then tail, then 2 next elements of GLOB.mob_list list for context:\n"
		msg += "---\n"
