	...
	if(istype(BadBoy))
		...
		switch(++BadBoy.failure_strikes[BadBoy.type])
			if(2)
				...
				FireHim = TRUE
			if(3)
				msg = "MC Notice: The [BadBoy.name] subsystem seems to be destabilizing the MC and will be put offline."
...
/datum/controller/master/Initialize(delay, init_sss, tgs_prime)
	...
	//if(world.system_type == MS_WINDOWS && CONFIG_GET(flag/toast_notification_on_init) && !length(GLOB.clients)) FIXME
	...
	//if(sleep_offline_after_initializations && CONFIG_GET(flag/resume_after_initializations)) FIXME
...
/datum/controller/master/proc/init_subsystem(datum/controller/subsystem/subsystem)
	...
	//rustg_time_reset(SS_INIT_TIMER_KEY) FIXME !!!
	var/start_timeofday = REALTIMEOFDAY

	var/result = subsystem.Initialize()

	// Capture end time
	//var/time = rustg_time_milliseconds(SS_INIT_TIMER_KEY) FIXME !!!
	var/time = (REALTIMEOFDAY - start_timeofday) / 10
	//var/seconds = round(time / 1000, 0.01) FIXME
	var/seconds = time

	// Always update the blackbox tally regardless.
	//SSblackbox.record_feedback("tally", "subsystem_initialize", time, subsystem.name) FIXME
	...
	var/chat_message = chat_warning ? "<span class='boldwarning'>[message]</span>" : "<span class='boldannounce'>[message]</span>"
...
/datum/controller/master/proc/StartProcessing(delay)
	...
	log_and_message_admins("MC Notice: MC crashed or runtimed, restarting")
	var/rtn2 = Recreate_MC()
	if(rtn2 <= 0)
		log_and_message_admins("MC Warning: Failed to recreate MC (Error code: [rtn2]), it's up to the failsafe now")
		Failsafe.defcon = 2
...
/datum/controller/master/proc/Loop(init_stage)
	...
	while (1)
		...
		//if(newdrift - olddrift >= CONFIG_GET(number/drift_dump_threshold)) FIXME
...
/datum/controller/master/proc/AttemptProfileDump(delay)
	...
	//SSprofiler.DumpFile(allow_yield = FALSE) FIXME
