...
/datum/component/topturfcrossed/Initialize()
	...
	parent.AddComponent(/datum/component/recursive_move) // Required if we want to be useful at all
	our_owner = parent
...
/datum/component/topturfcrossed/RegisterWithParent()
	RegisterSignal(our_owner, COMSIG_OBSERVER_MOVED, PROC_REF(handle_location_change))
	update_turf_signals(get_turf(our_owner))

/datum/component/topturfcrossed/UnregisterFromParent()
	UnregisterSignal(our_owner, COMSIG_OBSERVER_MOVED)
...
/datum/component/topturfcrossed/proc/update_turf_signals(atom/new_loc)
	...
	if(our_old_turf)
		UnregisterSignal(our_old_turf, COMSIG_MOVABLE_CROSS)
		our_old_turf = null
	// Only register the turf signal if we are inside something, otherwise we'd get DOUBLECROSSED
	if(new_loc && !isturf(new_loc))
		var/turf/find_new = get_turf(new_loc)
		if(find_new)
			RegisterSignal(find_new, COMSIG_MOVABLE_CROSS, PROC_REF(handle_turf_crossed))
			our_old_turf = find_new

/// Forwards the Cross() call from the turf to the object registered
/datum/component/topturfcrossed/proc/handle_turf_crossed(datum/source, atom/movable/crosser)
	SIGNAL_HANDLER
...
/obj/item/bikehorn/turfcross_testing
	name = "bikehorn of testing"
	desc = "honk if you're working correctly"

/obj/item/bikehorn/turfcross_testing/Initialize(mapload)
	. = ..()
