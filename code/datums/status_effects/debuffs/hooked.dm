...
/atom/movable/screen/alert/status_effect/hooked/Click()
	...
	if(!.)
		return
	if(!owner.incapacitated(INCAPACITATION_KNOCKOUT) && !owner.is_paralyzed() && (owner.last_resist_time + RESIST_COOLDOWN < world.time))
		return
...
/* //No slowdown for now. TODO: Port TG movespeed system.
