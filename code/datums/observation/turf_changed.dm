...
GLOBAL_DATUM_INIT(turf_changed_event, /decl/observ/turf_changed, new) //VOREStation Edit
...
/turf/ChangeTurf(var/turf/N, var/tell_universe, var/force_lighting_update, var/preserve_outdoors)
	...
	if(.)
		GLOB.turf_changed_event.raise_event(src, old_density, density, old_opacity, opacity) //VOREStation Edit