	...
	switch(A.stage)
		if(1, 2, 3)
			...
			if(power >= 1.5)
				...
				if(power >= 2)
					M.emote("mlem")
					addtimer(CALLBACK(M, TYPE_PROC_REF(/mob, emote), "mlem"), 6)
					addtimer(CALLBACK(M, TYPE_PROC_REF(/mob, emote), "mlem"), 12)
					addtimer(CALLBACK(M, TYPE_PROC_REF(/mob, emote), "mlem"), 18)
