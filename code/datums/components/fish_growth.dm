...
/datum/component/fish_growth/proc/finish_growing(obj/item/fish/source)
	...
	if(is_evo)
		...
		//if(isbasicmob(result))
			...
			addtimer(CALLBACK(result, TYPE_PROC_REF(/mob/living, hop_on_nearby_turf)), 0.1 SECONDS)

	if(is_evo || location == source.loc)
		var/message_verb = del_on_grow ? "grows into" : "lays"
		location.visible_message(span_notice("[source] [message_verb] \a [result]."), range = 3)
