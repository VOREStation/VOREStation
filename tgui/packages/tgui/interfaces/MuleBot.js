...
export const MuleBot = (props, context) => {
  ...
  const { suffix, power, load, locked, issilicon, auto_return, crates_only, hatch, safety } = data;
  return (
...
const MuleBotClosed = (props, context) => {
  const { act, data } = useBackend(context);
  const { suffix, power, load, locked, issilicon, auto_return, crates_only, open, safety } = data;
