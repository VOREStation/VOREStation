...
export const generateBellyString = (belly: Belly, index: number) => {
  ...
  result += `${name} (${display_name}) - (<span style="color: red;">${digest_brute}</span >/<span style="color: orange;">${digest_burn}</span>/<span style="color: blue;">${digest_oxy}</span>/<span style="color: green;">${digest_tox}</span>/<span style="color: purple;">${digest_clone}</span>) - ${ModeSpan[mode]} - ${ItemModeSpan[item_mode]}`;
  result += '</button></h2>';
